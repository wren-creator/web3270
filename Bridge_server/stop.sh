...
if command -v docker &>/dev/null; then
  ...
  echo -e "${RED}Error:${RESET} neither docker nor podman found." >&2; exit 1
fi
...
if $RUNTIME compose version &>/dev/null 2>&1; then
  ...
  echo -e "${RED}Error:${RESET} no compose tool found." >&2; exit 1
fi
...
for PORT in 3270 3271 3274 "$BRIDGE_PORT"; do
  ...
  if [ -n "$HOLDER" ]; then
    ...
    echo "$HOLDER" | awk '{print $1}' | xargs $RUNTIME rm -f
...
if [ -n "$DANGLING" ]; then
  ...
  echo "$DANGLING" | awk '{print $1}' | xargs $RUNTIME rm -f
...
if [ -n "$LOOPING" ]; then
  ...
  echo "$LOOPING" | awk '{print $1}' | xargs $RUNTIME rm -f
