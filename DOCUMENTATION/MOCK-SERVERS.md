...
- **User profiles** (`USRPRFS`): `QSECOFR`, `QSRV`, and `QSRVBAS` on their default password (`QSRVBAS` also holds `*ALLOBJ`); `APPADMIN` (an "application service account") quietly holding `*ALLOBJ`; `QPGMR`/`QSYSOPR`/`QUSER`/`QTMHHTTP` still enabled with passwords; `QYSPJ`, a `Q`-named `*ALLOBJ *SECADM` blend-in backdoor that IBM never ships (the Shipped Profile Audit flags it as an "irregular profile"); `QSYS` as the one shipped profile done right, `*DISABLED` and `PASSWORD(*NONE)` despite holding every special authority. The rest of the full IBM-supplied `Q*` set (~50 profiles, special authorities cross-checked against IBM i Security Reference SC41-5302) ship `PASSWORD(*NONE)` and are audit-compliant — present for enumeration realism, and the reason `WRKUSRPRF` now pages (Roll Up/Down). The Sign On screen is a user-enumeration oracle: `CPF1120` means the profile is not real, while `CPF1107`/`CPF1118`/`CPF1394` all confirm it exists — the RACF PROBE reports those as `EXISTS` and keeps sweeping. There is also a command-line-free enumeration path: `DSPJOB` (or User tasks → 6) → option `13` (Display library list) → `5` on `QSYS` → the paged Display Library panel, whose `*USRPRF` objects are every profile; `5` on one opens its `DSPUSRPRF` detail.
