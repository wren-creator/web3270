...
- **User profiles** (`USRPRFS`): `QSECOFR`, `QSRV`, and `QSRVBAS` on their default password (`QSRVBAS` also holds `*ALLOBJ`); `APPADMIN` (an "application service account") quietly holding `*ALLOBJ`; `QPGMR`/`QSYSOPR`/`QUSER`/`QTMHHTTP` still enabled with passwords; `QSYS` as the one shipped profile done right, `*DISABLED` and `PASSWORD(*NONE)` despite holding every special authority. The rest of the full IBM-supplied `Q*` set (~50 profiles, special authorities cross-checked against IBM i Security Reference SC41-5302) ship `PASSWORD(*NONE)` and are audit-compliant — present for enumeration realism, and the reason `WRKUSRPRF` now pages (Roll Up/Down).
